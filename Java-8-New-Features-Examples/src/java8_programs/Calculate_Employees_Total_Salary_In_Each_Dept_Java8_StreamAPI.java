...
public class Calculate_Employees_Total_Salary_In_Each_Dept_Java8_StreamAPI {
	...
	public static void main(String[] args) {
		...
		Map<String, Double> employeeMap = employeesList.stream().collect(Collectors
				.groupingBy(EmployeeClass::getDepartment, Collectors.summingDouble(EmployeeClass::getSalary)));
 ...
 * Collectors.summingDouble(EmployeeClass::getSalary) to calculate the sum of
 * the employees salaries in each department.
 * 
 * Once above results are collected to a map, we are iterating over this to
 * print the department name and employees total salary
 */