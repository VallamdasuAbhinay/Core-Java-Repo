...
public class Fetch_Max_Salary_Employee_Details_Using_min_function {
	...
	public static void main(String[] args) {
		List<EmployeeClass> employeesList = Arrays.asList(
				...
				new EmployeeClass("Urmila", "Vallamdasu", "HR", "Hyderabad", 1000),
				new EmployeeClass("Amarendar", "Vallamdasu", "Banking", "Hyderabad", 6000),
				new EmployeeClass("Shiva", "Kailas", "Exam", "Hyderabad", 6000));
		Optional<EmployeeClass> minSalaryEmployee = employeesList.stream()
				.min(Comparator.comparingDouble(EmployeeClass::getSalary));
		System.out.println("The Employee with the minimum salary are: ");
		minSalaryEmployee.ifPresent(emp -> System.out.println(emp));

		minSalaryEmployee.ifPresent(emp -> System.out.println(minSalaryEmployee.get()));
	}

}

/*
 * Here we are fetching the employee details having min salary by providing the
 ...
 * have min employee in the provided list.
 * 
 * To check if data is present or not, we have applied ifPresent() on Optional
 * object, then only we are printing it.
 * 
 * We can also use "minSalaryEmployee.get()" to print data in optional object.
 * 
 * Drawback
 * 
 * If there are multiple employees with same lowest salary, only one employee
