...
public class Sum_Of_Two_MultiDimensional_Arrays_Example {
	...
	public static void main(String[] args) {
		...
		int number_of_columns = multi_dim_array_a[0].length; //3 represents number of columns for row-0 in multi_dim_array_a
		int[][] arrays_sum = new int[number_of_rows][number_of_columns];
		...
		// traversing through the two multidimensional arrays and calculating the sum of their elements 
		// and storing in new multidimensional array
		System.out.println("\nData Representation in Matrix Sum:");
		for (int row = 0; row < number_of_rows; row++) {
			for (int column = 0; column < number_of_columns; column++) {
				arrays_sum[row][column] = multi_dim_array_a[row][column] + multi_dim_array_b[row][column];
			}
		}
		printArrayInMatrixFormat(arrays_sum);
	}
